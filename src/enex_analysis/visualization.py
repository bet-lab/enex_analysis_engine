...
def _draw_cycle_lines_and_annotations(
    ...
    def draw_segment(k1, k2):
        if not (np.isnan(pts_x[k1]) or np.isnan(pts_x[k2]) or np.isnan(pts_y[k1]) or np.isnan(pts_y[k2])):
            if points_are_close(pts_x[k1], pts_y[k1], pts_x[k2], pts_y[k2], tol=tol_atol):
                ax.plot(pts_x[k1], pts_y[k1], marker='o', linestyle='None', markerfacecolor=cycle_markerfacecolor, markeredgecolor=cycle_markeredgecolor, markeredgewidth=dm.lw(0), markersize=2.5, zorder=1)
            else:
                ax.plot([pts_x[k1], pts_x[k2]], [pts_y[k1], pts_y[k2]], color=line_color, linewidth=dm.lw(0), linestyle=':', zorder=1)
    ...
    points_list = []
    if points_are_close(pts_x['1_star'], pts_y['1_star'], pts_x['1'], pts_y['1'], tol_atol): points_list.append((pts_x['1'], pts_y['1'], "1"))
    else:
        if not (np.isnan(pts_x['1_star']) or np.isnan(pts_y['1_star'])): points_list.append((pts_x['1_star'], pts_y['1_star'], "1'"))
        if not (np.isnan(pts_x['1']) or np.isnan(pts_y['1'])): points_list.append((pts_x['1'], pts_y['1'], '1'))

    if points_are_close(pts_x['2'], pts_y['2'], pts_x['2_star'], pts_y['2_star'], tol_atol): points_list.append((pts_x['2'], pts_y['2'], "2"))
    else:
        if not (np.isnan(pts_x['2']) or np.isnan(pts_y['2'])): points_list.append((pts_x['2'], pts_y['2'], '2'))
        if not (np.isnan(pts_x['2_star']) or np.isnan(pts_y['2_star'])): points_list.append((pts_x['2_star'], pts_y['2_star'], "2'"))

    if points_are_close(pts_x['3_star'], pts_y['3_star'], pts_x['3'], pts_y['3'], tol_atol): points_list.append((pts_x['3'], pts_y['3'], "3"))
    else:
        if not (np.isnan(pts_x['3_star']) or np.isnan(pts_y['3_star'])): points_list.append((pts_x['3_star'], pts_y['3_star'], "3'"))
        if not (np.isnan(pts_x['3']) or np.isnan(pts_y['3'])): points_list.append((pts_x['3'], pts_y['3'], '3'))

    if not (np.isnan(pts_x['4']) or np.isnan(pts_y['4'])): points_list.append((pts_x['4'], pts_y['4'], '4'))
    ...
    for x_val, y_val, key in points_list:
        if key in ["1'", "2'"]:
            ax.plot(x_val, y_val, marker='o', markersize=2.5, markerfacecolor='white', markeredgecolor=color_sat_vap, markeredgewidth=dm.lw(0), zorder=2)
            continue
        elif key == "3'":
            ax.plot(x_val, y_val, marker='o', markersize=2.5, markerfacecolor='white', markeredgecolor=color_sat_liq, markeredgewidth=dm.lw(0), zorder=2)
            continue
            
        ax.plot(x_val, y_val, marker='o', markersize=2.5, markerfacecolor=cycle_markerfacecolor, markeredgecolor=cycle_markeredgecolor, markeredgewidth=dm.lw(0), zorder=2)
        ...
        if fig is not None:
            ...
            ax.annotate(text_str, (x_val, y_val), xytext=(dx, dy), textcoords='offset points', ha=ha, va=va, fontsize=font_sz)
...
def plot_th_diagram(
    ...
    _draw_cycle_lines_and_annotations(ax, pts_x, pts_y, is_on, color1, color2, line_color, color3, color4, tol_atol=0.05)
    ...
    if pd.notna(T_tank):
        ...
        ax.text(0.0, T_tank, f'Tank: {T_tank:.1f}°C', color='oc.red5', ha='left', va='bottom', transform=transform, fontsize=font_sz)

    if pd.notna(T0):
        ax.axhline(y=T0, color='oc.orange5', linestyle=':', linewidth=dm.lw(0))
        offset = dm.make_offset(4, -4, ax.figure) if ax.figure else None
        transform = trans + offset if offset else trans
        ax.text(0.0, T0, f'Outdoor: {T0:.1f}°C', color='oc.orange5', ha='left', va='top', transform=transform, fontsize=font_sz)
    ...
    legend_handles = [
        ...
        ax.plot([], [], color=line_color, linewidth=dm.lw(0), marker='o', linestyle=':', markersize=2.5, markerfacecolor=color3, markeredgecolor=color3, markeredgewidth=dm.lw(0), label='Ref. cycle')[0]
    ]
    ax.legend(handles=legend_handles, loc='upper right', handlelength=1.5, labelspacing=0.5, columnspacing=2, ncol=3, frameon=False)
