...
class ElectricBoiler:
    ...
    def analyze_dynamic(
        ...
        for n in tqdm(
            range(tN),
            desc="ElectricBoiler Simulating",
        ):
            t_s: float = time[n]
            hr: float = t_s * cu.s2h
            ...
            is_on, result, Q_heat_source = self._determine_heater_state(
                ctx,
                is_on_prev,
            )
            is_on_prev = is_on
    ...
    def postprocess_exergy(
        ...
        X_in_tank = df["E_heater [W]"].fillna(0) + df[
            "X_tank_w_in [W]"
        ].fillna(0)
        if "X_uv [W]" in df.columns:
        ...
        df["X_eff_sys [-]"] = df["X_tank_w_out [W]"].fillna(0) / df[
            "X_tot [W]"
        ].replace(0, np.nan)
